...
function WeatherIcon({ symbol }: { symbol: WeatherSymbol }) {
  ...
  const rainStart = 32 - ((rainCount - 1) * 8) / 2
  return <svg viewBox="0 0 64 64" width="52" height="52" focusable="false">
    {sun && <g transform={symbol === 'sun' ? 'translate(-8 11)' : undefined} stroke="#e7aa22" strokeWidth="2.5" strokeLinecap="round"><circle cx="40" cy="21" r="9" fill="#ffd35a" />{Array.from({ length: 8 }, (_, i) => <path key={i} d="M40 5v3" transform={`rotate(${i * 45} 40 21)`} />)}</g>}
    ...
    {Array.from({ length: rainCount }, (_, i) => <path key={i} d={`M${rainStart + i * 8} 48l-3 ${symbol === 'stormRain' ? 10 : 6}`} stroke="#328bc5" strokeWidth="2.7" strokeLinecap="round" />)}
