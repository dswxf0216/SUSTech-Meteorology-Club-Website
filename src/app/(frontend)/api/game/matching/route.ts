import { gameAction, leaderboard, startGame } from '@/utilities/matchingGame'

export const runtime = 'nodejs'
export const dynamic = 'force-dynamic'
const limits = new Map<string, { count: number; until: number }>()
export async function GET() {
  try { return Response.json({ scores: await leaderboard() }, { headers: { 'Cache-Control': 'no-store' } }) }
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (body.action === 'start') {
      ...
      if (++limit.count > 20) return Response.json({ error: '开始次数过多，请一分钟后再试。' }, { status: 429 })
      limits.set(ip, limit)
      ...
      return Response.json(await startGame(body.nickname.trim()), { headers: { 'Cache-Control': 'no-store' } })
    }
    if (!['match', 'next', 'state'].includes(body.action) || typeof body.sessionId !== 'string') return Response.json({ error: '游戏请求无效。' }, { status: 400 })
    return Response.json(await gameAction(body.sessionId, body.leftId, body.rightId, body.action === 'next', body.action === 'state'), { headers: { 'Cache-Control': 'no-store' } })
  } catch (error) {
    const message = error instanceof Error ? error.message : ''
    const safe = /^(游戏|请选择|该选项|请先|请求繁忙)/.test(message)
