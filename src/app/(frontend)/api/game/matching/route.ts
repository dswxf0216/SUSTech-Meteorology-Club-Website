...
import { gameIdentity as identity } from '@/utilities/gameIdentity'
...
export async function GET(request: Request) {
  try {
    ...
    return Response.json({ error: '排行榜暂时无法读取，请稍后刷新。' }, { status: 503 })
  }
}
export async function POST(request: Request) {
  ...
  try {
    ...
    if (body.action === 'start') {
      ...
      return Response.json(
        await startDeviceGame(body.nickname.trim(), who.admin ? undefined : who.deviceId),
        { headers: who.headers },
      )
    }
    if (!['match', 'next', 'state'].includes(body.action) || typeof body.sessionId !== 'string')
    ...
    return Response.json(
      { error: safe ? message : '游戏服务暂时不可用，请重试。' },
      { status: safe ? 400 : 503 },
    )
  }
}
