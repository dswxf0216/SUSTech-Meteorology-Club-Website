...
登录页的“申请编辑账号”链接指向 `/register`。申请人自行填写密码（12–128位），系统使用 Payload 内置密码哈希保存，不发送邮件或生成登录会话。公开申请强制为“编辑 / 待审批”，不接受请求中指定的角色或审批状态。

管理员进入“管理员”列表，筛选“审批状态 = 待审批”，查看姓名、邮箱、申请说明，将状态改为“已批准”或“已拒绝”后保存。获批者使用自设密码登录；编辑不能创建账号或审批申请。将账号改回待审批或拒绝会撤销其已有会话。历史账号经数据库迁移默认为已批准，保留原角色。

申请接口校验同源请求、限制请求体大小，并按来源地址每小时限制5次尝试；限流状态仅保存在单个服务进程内，重启会重置，多实例部署时需升级为共享限流。无需邮件服务；审批结果由管理员线下告知。此流程没有邮箱验证，不应仅凭填写的邮箱认定申请人身份。
